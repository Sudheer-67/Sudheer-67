...
(() => {
    ...
    selectCountry({
        target: {
            value: countries[0].country_code
        }
    });
...
function saveLeadDetails(event, courseId, techStack, classMode) {
    ...
    xhr.open('POST', 'https://api.app.crm.skill-capital.com/api/v1/leads');
    ...
    xhr.onload = function() {
        if (xhr.status >= 200 && xhr.status < 300) {
            ...
            document.getElementById('countrySelect').value = countries[0]['country_code'];
            window.location.href = 'https://www.nexadesign.ai/thankyou.html';
            //alert('Lead details saved successfully');
        } else {
            // Request failed
            const error = JSON.parse(xhr.responseText);
            if (error && error.error === 'Phone number already exists') {
                alert('Phone number already exists, Please use different phone number')
            ...
            console.error('Error saving lead details:', xhr.responseText);
